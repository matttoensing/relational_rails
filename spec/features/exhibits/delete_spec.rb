...
RSpec.describe 'delete button' do
  ...
  it 'user can click on button to delete exhibit' do
    ...
    exhibit2 = museum.exhibits.create!(title: "The Alwafai Exhibit", person_limit: 50, photos: true, flash: true)

    visit "/exhibits/#{exhibit1.id}"
    click_on "Delete: #{exhibit1.title}"

    expect(current_path).to eq("/exhibits")
    expect(page).to_not have_content("The Toensing Exhibit")
    expect(page).to have_content("The Alwafai Exhibit")
  end
end
